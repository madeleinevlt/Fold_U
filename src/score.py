...
class Score:
    ...
    def write_score(self, res_path, nb_pdb, alignment_dict):
        ...
        for _, ali_score, thr_score, ss_score, _, name, benchmark in sorted(self.iterator):
            scores_df.loc[name] = [benchmark, int(ali_score), int(thr_score), int(ss_score)]

        # The first row is removed because it corresponds to the query
        scores_df = scores_df.drop(scores_df.index[0])
        # Normalization of the scores
        for index in ['alignment', 'threading', 'secondary_structure']:
