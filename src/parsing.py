...
def set_benchmark(benchmark_file, foldrec_file, alignment_dict):
    """
        Extract the line of the benchmark file containing the benchmarks of the study query.
        Each bechmark template have an assigned structure corresponding to the degree of
        similarity with the query ("Family", "Superfamily" or "Fold").
        For a given benchmark template object, the structure is store in the benchmark item.

        Args:
            benchmark_file (str): The path of the benchmark file containing bencharks
                                  for each foldrec file.
            foldrec_file (str): The path of the foldrec file.
            alignment_dict (dictionary): A dictionary with key = template name and
                                         value = an Alignment object.
    ...
    with open(benchmark_file, "r") as file:
        for line in file:
            # Only the line containing the query name is extract
            if query_name.group(1) in line:
                # List containing for each benchmark the template name and the associated structure
                template_structure = line[0:-1].split(": ")[1].split(", ")
                for i in template_structure:
                    # The structure of the benchmark template is stored in the benchmark item
                    template = i.split(" ")[0]
                    structure = i.split(" ")[1]
